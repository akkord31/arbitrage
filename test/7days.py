...
def get_data(symbol, timeframe='5m', hours=None, days=None):
    all_data = []
    if hours:
...
# Загружаем данные BTC и ETH за последние 7 дней(5 минута)
btc_data_24h = get_data('BTC/USDT', '5m', days=7)
eth_data_24h = get_data('ETH/USDT', '5m', days=7)
...
# Объединяем данные по времени (для 7 дней)
...
# **Пересчет BTC в "ETH-единицах" для 7дней**
...
# **Средний коэффициент BTC/ETH за 7 дней**
...
axes[0, 0].set_title('Цена Bitcoin (7 дней, 5м)')
axes[0, 0].legend()
axes[0, 0].grid(True)
...
axes[0, 1].set_title('Цена Ethereum (7 дней, 5м)')
axes[0, 1].legend()
axes[0, 1].grid(True)
...
axes[1, 0].set_title('BTC в ETH-единицах и ETH (7 дней, 5м)')
axes[1, 0].legend()
axes[1, 0].grid(True)
...
axes[2, 0].set_title('Процентная разница BTC (норм.) и ETH (норм.) ( 7 дней, 5м)')
axes[2, 0].legend()
axes[2, 0].grid(True)

...
def get_data(symbol, timeframe='15m', days=90):  # Изменено на 90 дней
    all_data = []
    since = int((datetime.utcnow() - timedelta(days=days)).timestamp() * 1000)  # Время начала (90 дней назад)
...
# Загружаем данные BTC и ETH за 90 дней
btc_data = get_data('BTC/USDT', '15m', days=90)  # Изменено на 90 дней
eth_data = get_data('ETH/USDT', '15m', days=90)  # Изменено на 90 дней
...
# **Средний коэффициент BTC/ETH за 90 дней**
...
# Вычисляем процентную разницу между нормализованным BTC и ETH
...
axes[0, 0].set_title('Цена Bitcoin (90 дней, 15м)')
axes[0, 0].legend()
axes[0, 0].grid(True)
...
axes[0, 1].set_title('Цена Ethereum (90 дней, 15м)')
axes[0, 1].legend()
axes[0, 1].grid(True)
...
axes[1, 0].set_title('BTC в ETH-единицах и ETH (90 дней, 15м)')
axes[1, 0].legend()
axes[1, 0].grid(True)
...
axes[1, 1].set_title('Нормализованные данные BTC и ETH (90 дней, 15м)')
axes[1, 1].legend()
axes[1, 1].grid(True)
...
axes[2, 0].set_title('Процентная разница BTC (норм.) и ETH (норм.) (90 дней, 15м)')
axes[2, 0].legend()
axes[2, 0].grid(True)
